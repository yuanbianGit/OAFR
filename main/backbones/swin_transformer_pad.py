...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        # print(relative_position_bias.shape) torch.Size([4, 49, 49])
        # print(attn.shape) torch.Size([1600, 4, 49, 49])
...
#         #             count += 1
#         #     attn_mask = attn_mask.view(shifted_x.size(1) // self.window_size, self.window_size,
#         #                                shifted_x.size(2) // self.window_size, self.window_size)
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(H_ori)
        #
        # H_1 = int(224 / (224 / H_ori))
        # W_1 = int(224 / (224 / W_ori))
        #
        H_1 = int(256/(224/H_ori))
        W_1 = int(128/(224/W_ori))
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(L)
        # H, W = self.input_resolution
        H = int(math.sqrt(int(L/2))*2)
        W = int(math.sqrt(int(L/2))*1)
        # print(H) #64
        # print(W) #32
        assert L == H * W, "input feature has wrong size"
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None):
        ...
        patch_size = (4,4) #(4,4)
        patches_resolution = [img_size[0] // patch_size[0], img_size[1] // patch_size[1]] # (64,32)
...
class SwinTransformer_pad(nn.Module):
    r""" Swin Transformer
    ...
    def forward_features(self, x):
        ...
        x = torch.flatten(x, 1)
        return x
    def forward_features_by(self, x):
        x = self.patch_embed(x)
        if self.ape:
            x = x + self.absolute_pos_embed
        x = self.pos_drop(x)
